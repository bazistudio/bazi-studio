...
export default function ProjectCard({ project }: { project: any }) {
  // 1. Find cover image (prefer role === 'cover', fallback to first image, fallback to placeholder)
  const coverMedia = 
  ...
  const coverUrl = coverMedia?.url || "/images/placeholder-project.svg";
  
  // 2. Extract category
  const categoryName = project.categories?.name || "Portfolio";
      ...
      <div className="relative aspect-[16/10] overflow-hidden bg-muted/30">
        ...
        {coverMedia?.url ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center text-muted-foreground bg-muted/40 p-6 text-center">
            ...
            <span className="text-xs font-medium tracking-wide uppercase">{project.title}</span>
          </div>
        )}
