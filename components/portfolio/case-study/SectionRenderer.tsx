...
const CodeShowcase = ({ content, title }: { content: string, title?: string }) => (
  <div className="my-12">
    {title && <h4 className="text-sm font-mono text-primary mb-3 flex items-center gap-2"><Terminal size={14}/> {title}</h4>}
    <div className="bg-[#0A0A0A] border border-border/50 rounded-xl p-6 overflow-x-auto font-mono text-sm shadow-2xl shadow-black/50">
      <pre><code className="text-blue-300">{content}</code></pre>
    </div>
  </div>
);

const ArchitectureBlock = ({ content, title }: { content: string, title?: string }) => (
  <div className="my-12 glass-panel border border-purple-500/20 p-8 md:p-12 rounded-3xl bg-gradient-to-br from-purple-500/5 to-transparent relative overflow-hidden">
    <div className="absolute top-0 right-0 p-8 opacity-10 pointer-events-none">
      <Layout size={100} className="text-purple-500" />
    </div>
    <div className="relative z-10">
      {title && <h4 className="text-xl font-bold text-purple-400 mb-4 flex items-center gap-2"><Layout size={20}/> {title}</h4>}
...
const QuoteBlock = ({ content, title }: { content: string, title?: string }) => (
  <blockquote className="my-16 border-l-4 border-primary pl-6 md:pl-10 py-2">
    ...
    {title && <footer className="text-sm font-semibold text-primary mt-6 tracking-widest uppercase">— {title}</footer>}
  </blockquote>
);
